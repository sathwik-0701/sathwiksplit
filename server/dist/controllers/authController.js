...
const forgotPassword = async (req, res) => {
    try {
        ...
        user.otpExpires = new Date(Date.now() + 10 * 60 * 1000);
        await user.save();
        const emailSent = await (0, emailService_1.sendPasswordResetEmail)(normalizedEmail, otp);
        if (!emailSent) {
            console.error(`❌ forgotPassword: OTP email to ${normalizedEmail} FAILED to send. Check BREVO_API_KEY and sender email verification.`);
            res.status(500).json({
                success: false,
                message: 'Failed to send OTP email. Please check server email setup or try again later.',
            });
            return;
        }
        res.status(200).json({
